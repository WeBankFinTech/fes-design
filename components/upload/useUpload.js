...
export default (props, { emit }) => {
    ...
    function abort(file) {
        ...
        if (file) {
            ...
            Object.keys(_requestList).forEach((uid) => {
                if (_requestList[uid]) _requestList[uid].abort();
                delete _requestList[uid];
            });
        }
    }
    ...
    function onProgress({ event, rawFile }) {
    ...
    function onSuccess({ response, rawFile }) {
        const file = getFile(rawFile, uploadFiles.value);
        if (!file) return;
        file.status = 'success';
    ...
    function onError({ error, rawFile }) {
