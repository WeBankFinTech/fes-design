import { defineComponent, watch, onMounted, onBeforeUnmount, nextTick } from 'vue';
...
export default defineComponent({
    ...
    setup(props, { slots, emit, expose }) {
        const { prefixCls, wrapperRef, wrapperClass, carouselStyle, slideChildren, carouselState, prev, next, setActiveItem, resetItemPosition } =
            useCarousel(props);

        const defaultNodes = slots.default();
        defaultNodes.forEach((childNode) => {
            if (childNode.type?.name !== CAROUSEL_ITEM_NAME) {
                console.error(`[${CAROUSEL_NAME}]: ${CAROUSEL_NAME} 子标签必须是 ${CAROUSEL_ITEM_NAME}`);
            }
        });
        ...
        const startTimer = () => {
            if (props.interval <= 0 || !props.autoplay || carouselState.timer) return;
            carouselState.timer = setInterval(() => play(), props.interval);
        };
        ...
        const handleMouseEnter = (event) => {
            ...
            carouselState.hover = true;
            if (props.pauseOnHover) pauseTimer();
        };

        const handleMouseLeave = (event) => {
            event.stopPropagation();
            carouselState.hover = false;
            startTimer();
        };
        ...
        onMounted(() => {
            nextTick(() => {
                ...
                carouselState.containerWidth = wrapperRef.value.offsetWidth;
        ...
        // render
        return () => (
            <div ref={wrapperRef} class={wrapperClass.value} onMouseenter={handleMouseEnter} onMouseleave={handleMouseLeave}>
                <div class={`${prefixCls}-slides`}>
                    <Arrow hover={carouselState.hover} activeIndex={carouselState.activeIndex} />
                    <div class={`${prefixCls}-list`} style={carouselStyle.value}>
                        {slots.default?.()}
                    </div>
                </div>
                <Indicator
